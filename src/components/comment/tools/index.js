...
import { mRatePost } from "../../../graphql/mutation";
...
class Tools extends React.Component<any, State> {
  ...
  toggleThumbup = async () => {
    const { isLoggedin, me, like } = this.state;
  ...
  render() {
    ...
    const { gammatags, rate } = this.props.post;
    ...
    return (
      <div className="tools" ref={wrapper => (this.wrapper = wrapper)}>
        ...
        <div className="">
          <Icon
            className="icon"
            name={like === 1 ? "heart_fill" : "heart_outline"}
            size={24}
          ...
          <Icon
            className="icon"
            name={like === 2 ? "thumb_down" : "thumb_down_outline"}
            size={24}
